...
connection.query(("CREATE TABLE teams (team_id INT PRIMARY KEY AUTO_INCREMENT, team_name VARCHAR(50), team_initials VARCHAR (10), team_formedYear INT, team_stadium VARCHAR(100), team_country VARCHAR(50));"), (err, result) => {
    if (err){
        console.log(err);
    }
    else{
...
connection.query(("CREATE TABLE competitions (competition_id INT PRIMARY KEY AUTO_INCREMENT, competition_name VARCHAR(100), competition_format VARCHAR(50), competition_winner_team_id INT, competition_season VARCHAR(50), FOREIGN KEY(competition_winner_team_id) REFERENCES teams(team_id));"), (err, result) => {
    if (err){
        console.log(err);
    }
    else{
...
connection.query(("CREATE TABLE athletes (athlete_id INT PRIMARY KEY AUTO_INCREMENT, athlete_name VARCHAR(100) NOT NULL, athlete_birthDate DATE NOT NULL, athlete_height DECIMAL (3, 2), athlete_weight DECIMAL (5, 2), athlete_nationality VARCHAR (50) NOT NULL, athlete_position VARCHAR (50), athlete_team_id INT, FOREIGN KEY(athlete_team_id) REFERENCES teams(team_id));"), (err, result) => {
    if (err){
        console.log(err);
    }
    else{
...
connection.query(("CREATE TABLE games (game_id INT PRIMARY KEY AUTO_INCREMENT, game_house_team_id INT, game_visiting_team_id INT, game_result VARCHAR (10));"), (err, result) => {
    if (err){
        console.log(err);
    }
    else{
