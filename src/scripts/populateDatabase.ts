...
function fetchCompetitions() {
    return new Promise((resolve, reject) => {
        ...
        fetch("https://www.thesportsdb.com/api/v1/json/3/all_leagues.php")
            ...
            .then((data) => {
                let basketballLeagues: Array<{ idLeague: Number, strLeague: String, strSport: String }> = data.leagues;
                basketballLeagues = basketballLeagues.filter((league: { idLeague: Number, strLeague: String, strSport: String }) => league.strSport == "Basketball");
                console.log(basketballLeagues);
                basketballLeagues.forEach((league: { idLeague: Number, strLeague: String, strSport: String }) => {
                    connection.query<mysql.ResultSetHeader>((`INSERT INTO competitions (competition_id, competition_name) VALUES (${league.idLeague},'${league.strLeague}');`), (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else {
